...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Convert the Gemini-style request used by Dashboard.tsx
    // into the message format expected by the KIE provider.
    ...
    const topP =
      typeof body.topP === 'number' ? body.topP : 0.95;

    const maxTokens =
      typeof body.maxTokens === 'number' ? body.maxTokens : 8192;

    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 55_000);

    let providerResult: { data: any; response: Response };
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return sendJson(res, 401, {
          error: 'KIE API token tidak valid atau tidak memiliki akses.',
        });
      }

      if (response.status === 429) {
        return sendJson(res, 429, { error: `KIE rate limit: ${message}` });
      }

      if (response.status >= 500) {
        return sendJson(res, 502, { error: `KIE server error: ${message}` });
      }

      return sendJson(res, 400, { error: `KIE: ${message}` });
    }

    // Keep the response contract expected by Dashboard.tsx.
    // KIE: choices[0].message.content
    // Dashboard: candidates[0].content.parts[0].text
    ...
    if (/abort|timeout/i.test(message)) {
      return sendJson(res, 504, {
        error: 'Request ke KIE timeout. Coba Generate lagi.',
      });
    }

    if (/KIE_API_KEY belum dikonfigurasi/i.test(message)) {
      return sendJson(res, 500, { error: message });
    }

    if (/KIE mengembalikan response yang tidak valid/i.test(message)) {
